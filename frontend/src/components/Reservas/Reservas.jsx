...
export default function Reservas() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      const tipoReservaIds = tipoReservaFiltrada.map(tr => tr.idTipoReserva);
      const reservas = await reservasService.getReservas();

      let reservaFiltrada = reservas.filter(reserva => tipoReservaIds.includes(reserva.idTipoReserva));

      if (!showInactive) {
        reservaFiltrada = reservaFiltrada.filter(reserva => reserva.activo);
      }
